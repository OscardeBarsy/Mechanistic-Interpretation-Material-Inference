...
def normalise_tensor(tensor: torch.Tensor) -> torch.Tensor:
    ...
    if t.is_cuda:
        t = t.cpu()
    max_abs = torch.abs(t).max()
    if max_abs.item() == 0.0:
        return t
    return t / max_abs


def resize_all(tensor: torch.Tensor, expected_in: int = 15, expected_out: int = 11) -> torch.Tensor:
    """
    Expects [n_layers, seq_len]. Merges columns by a fixed plan for 15->11.
    If already 11, returns as-is. Else raises a clear error.
    Returns a CPU tensor.
    """
    x = tensor.detach()
    if x.is_cuda:
        x = x.cpu()
    assert x.dim() == 2, f"resize_all expects [layers, seq], got {tuple(x.shape)}"
    L, S = x.shape

    def merge_and_delete(mat: torch.Tensor, pre: int, post: int) -> torch.Tensor:
        S = mat.shape[1]
        if not (0 <= pre < S and 0 <= post < S):
            raise IndexError(f"resize_all: pre/post out of range (pre={pre}, post={post}, S={S})")
        mat = mat.clone()
        mat[:, pre] = mat[:, pre] + mat[:, post]
        return torch.cat([mat[:, :pre], mat[:, pre+1:]], dim=1)

    if S == expected_out:
        return x
    elif S == expected_in:
        # Your original merge plan: (11<-12), (10<-11), (9<-10), (4<-5)
        x = merge_and_delete(x, 11, 12)
        x = merge_and_delete(x, 10, 11)
        x = merge_and_delete(x, 9, 10)
        x = merge_and_delete(x, 4, 5)
        if x.shape[1] != expected_out:
            raise ValueError(f"resize_all: after merges, got width {x.shape[1]} (expected {expected_out})")
        return x
    else:
        raise ValueError(
            f"resize_all: unexpected seq_len={S}. Expected {expected_in} (pre-merge) or {expected_out} (already merged). "
            f"Tokenization/template likely changed; update the merge plan or your labels accordingly."
        )


def compress_to_labels(x: torch.Tensor, n_out: int = 11, mode: str = "mean") -> torch.Tensor:
    """
    Compress a [n_layers, seq_len] tensor to [n_layers, n_out].
    - If seq_len == n_out: return a clone.
    - If seq_len >  n_out: pool equal-width bins using mean (or sum).
    - If seq_len <  n_out: upsample by nearest neighbor selection.

    Args:
        x: [L, S] torch tensor (CPU or CUDA)
        n_out: desired number of columns (e.g., 11)
        mode: 'mean' or 'sum' when pooling

    Returns:
        [L, n_out] tensor on CPU.
    """
    assert x.dim() == 2, f"compress_to_labels expects [L,S], got {tuple(x.shape)}"
    t = x.detach()
    if t.is_cuda:
        t = t.cpu()
    L, S = t.shape
    ...
    if S > n_out:
        # Pool equal-width bins
        edges = torch.linspace(0, S, steps=n_out + 1, dtype=torch.long)
        # Make sure edges are strictly increasing and within [0,S]
        edges[0] = 0
        edges[-1] = S
        cols = []
        for i in range(n_out):
            ...
            if b <= a:
                # emergency fallback: take the nearest single column
                a = min(a, S - 1)
                seg = t[:, a:a+1]
            else:
                seg = t[:, a:b]
            cols.append(seg.mean(dim=1, keepdim=True) if mode == "mean" else seg.sum(dim=1, keepdim=True))
        out = torch.cat(cols, dim=1)  # [L, n_out]
        return out

    else:  # S < n_out
        # Nearest-neighbor expand
        idx = torch.linspace(0, S - 1, steps=n_out).round().long()  # [n_out]
...
def patching_residual(model, corrupted_tokens, clean_cache, patching_metric,
    ...
    # Keep results on CPU (small tensor; avoids any GPU alloc here)
    results = torch.empty(model.cfg.n_layers, T, dtype=torch.float32, device="cpu")
    ...
    for layer in range(model.cfg.n_layers):
        for pos in range(T):
            ...
            # write the averaged score for this (layer, pos)
            results[layer, pos] = accum / B
